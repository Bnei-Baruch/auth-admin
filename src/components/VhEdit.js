...
class VhEdit extends Component {
    ...
    render() {
        const {open_edit, selected_user, status_from_order, payments} = this.props;
                ...
                <Modal.Content>
                    <Form size='large'>
                        ...
                        <Form.Field
                            id='status'
                            control={Button}
                            content={<Table compact='very' structured unstackable singleLine celled>
                                <Table.Header>
                                    <Table.Row>
                        ...
                        <Form.Field
                            id='status'
                            control={Button}
                            content={<Table selectable compact='very' className="ingest_table">
                                {payments.data?.map((p,i) => {
                                        return (
                                            <Table.Row key={i} className='table_header'>
                                                {Object.keys(p)?.map(s => {
                                                        if (s === "status") return
